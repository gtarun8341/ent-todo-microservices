...
type Session struct {
	...
	// ExpiryAt holds the value of the "expiryAt" field.
...
func (*Session) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case session.FieldID, session.FieldExpiryAt:
	...
	for i := range columns {
		...
		case session.FieldExpiryAt:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field expiryAt", values[i])
			} else if value.Valid {
				s.ExpiryAt = value.Int64
			}
		case session.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field user_id", values[i])
			} else if value.Valid {
				s.user_id = new(uuid.UUID)
				*s.user_id = *value.S.(*uuid.UUID)
			}
		default:
...
func (s *Session) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("expiryAt=")
	builder.WriteString(fmt.Sprintf("%v", s.ExpiryAt))
	builder.WriteByte(')')
